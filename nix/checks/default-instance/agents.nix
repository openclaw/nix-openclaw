...
let
  inherit (helpers)
    ...
    usesAgentEntries
    ;
  inherit (import ./agent-cases.nix { inherit lib explicitOwnership; })
    authoredRosterCases
  ...
  emptyRosterChecks = lib.optionals usesAgentEntries (
    map
      (
        pinAgentDefaults:
        let
          ...
          // lib.optionalAttrs pinAgentDefaults {
  ...
  explicitWorkspaceCheck =
    if
      explicitWorkspaceConfig.agents != (
        {
          defaults.workspace = "/tmp/authored-workspace";
        }
        // lib.optionalAttrs usesAgentEntries { entries.main = { }; }
      )
    then
  ...
  authoredRosterChecks = lib.optionals usesAgentEntries (
    map (
      agents:
      let
  ...
  canonicalAgentIdChecks = lib.optionals usesAgentEntries (
    map (
      { entries, expected }:
      let
  ...
  invalidRosterChecks = lib.optionals usesAgentEntries (
    map
      (
        key:
        requireEvalFailure "unsafe agent key"
          (moduleEval {
    ...
    ++
      map
        (
          keys:
          requireEvalFailure "normalized agent collision ${builtins.toJSON keys}"
            (moduleEval {
              config.agents = explicitOwnership // {
                entries = lib.genAttrs keys (_: { });
              };
            }).config.home.activation
        )
        [
    ...
    ++ [
      (requireEvalFailure "malformed roster value" (
        generatedConfig (moduleEval {
        ...
        if usesAgentEntries then
        ...
        else
          {
            list = [
              ...
              {
                id = "writer";
                workspace = "/tmp/overridden-writer";
              }
            ];
          };
    };
