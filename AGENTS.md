...
- `runtimePackages` must not enable a runtime plugin, choose a Codex app-server command, or create Codex filesystem state by itself.
- If a user sets `tools.exec.pathPrepend` globally or per agent, keep those explicit user paths ahead of generated `runtimePackages` paths. Generated runtime paths should fill in missing tools, not silently shadow an operator's chosen command path.
...
- If a user provides `plugins.entries.codex.config.appServer.command` or `OPENCLAW_CODEX_APP_SERVER_BIN`, OpenClaw is no longer using the Nix Codex launcher. Do not create or update `$CODEX_HOME/home/.nix-profile/bin` for that case unless there is a reviewed upstream contract change.
- If a user sets `plugins.entries.codex.config.appServer.transport = "websocket"`, OpenClaw connects to an already-running app-server. Do not export `OPENCLAW_CODEX_APP_SERVER_BIN` or `OPENCLAW_CODEX_APP_SERVER_ARGS` for a local stdio launch in that case.
- Do not create or update `$CODEX_HOME/home/.nix-profile/bin` during Home Manager activation. Activation does not know which inherited environment OpenClaw will see when the gateway starts, and upstream treats `OPENCLAW_CODEX_APP_SERVER_BIN` as a runtime command override.
- Upstream OpenClaw sets per-agent `CODEX_HOME` for the Codex app-server and normally inherits process `HOME`. The Nix launcher intentionally sets `HOME=$CODEX_HOME/home` because Codex `command/exec` rebuilds command PATH around `$HOME/.nix-profile/bin`; only prepending `$CODEX_HOME/home/.nix-profile/bin` in the launcher process is not enough. Keep this documented as a Nix-managed Codex upgrade behavior, not an upstream default.
